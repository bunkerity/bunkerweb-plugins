...
function _M.parse_multipart(body, boundary)
	...
	while true do
		...
		if he then
			...
			-- RFC 5987 filename*= forms (the same matcher as clamav.lua).
...
-- Parse a single ClamAV INSTREAM response line into (verdict, unscannable).
-- "<name> FOUND" -> (name, false); any clean/OK line -> ("clean", false); a
-- StreamMaxLength notice -> ("clean", true) so the caller skips the file without
-- caching or denying (mirrors the inline behavior of clamav:scan()).
function _M.parse_instream_result(line)
	if type(line) ~= "string" then
		return "clean", false
	end
	if line:find("INSTREAM size limit exceeded", 1, true) then
		return "clean", true
	end
	local sig = line:match("^stream: (.*) FOUND$")
	if sig then
		return sig, false
	end
	return "clean", false
