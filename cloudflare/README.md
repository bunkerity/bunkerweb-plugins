...
| `CLOUDFLARE_AUTO_REAL_IP`               | `yes`                                                                           | multisite | no       | Configure NGINX real IP handling for Cloudflare; disable when the core Real IP plugin owns it.                                         |
| `CLOUDFLARE_REAL_IP_HEADER`             | `CF-Connecting-IP`                                                              | multisite | no       | Header carrying the real client IP: CF-Connecting-IP, True-Client-IP (Enterprise), or CF-Connecting-IPv6 (Pseudo-IPv4).                |
| `CLOUDFLARE_STRIP_SPOOFED_HEADERS`      | `yes`                                                                           | multisite | no       | Strip client-supplied Cloudflare headers from untrusted connections to prevent spoofing.                                               |
| `CLOUDFLARE_AUTHENTICATED_ORIGIN_PULLS` | `no`                                                                            | multisite | no       | Require Cloudflare's origin-pull client certificate; incompatible with the core mTLS plugin on the same server.                        |
| `CLOUDFLARE_AOP_MODE`                   | `log`                                                                           | multisite | no       | Log mode warns about invalid Cloudflare client certificates; enforce mode denies those connections.                                    |
...
| `USE_CLOUDFLARE_EDGE_BAN_SYNC`          | `no`                                                                            | global    | no       | Sync active BunkerWeb bans to a Cloudflare account IP List; requires Redis and an account-scoped token with Account Filter Lists:Edit. |
