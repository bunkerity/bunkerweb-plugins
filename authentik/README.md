...
| `AUTHENTIK_URL`                   |                                                                                                                       | multisite | no       | Base URL of the Authentik outpost; required when USE_AUTHENTIK is enabled.                             |
| `AUTHENTIK_OUTPOST_PATH`          | `/outpost.goauthentik.io`                                                                                             | multisite | no       | Local path exposing Authentik outpost endpoints; must start with /.                                    |
| `AUTHENTIK_SSL_VERIFY`            | `yes`                                                                                                                 | multisite | no       | Verify the Authentik outpost TLS certificate for auth checks and SSO proxying.                         |
| `AUTHENTIK_TIMEOUT`               | `5000`                                                                                                                | global    | no       | Timeout (ms) for the Lua auth subrequest to the Authentik outpost.                                     |
| `AUTHENTIK_PROXY_BUFFER_SIZE`     | `32k`                                                                                                                 | multisite | no       | NGINX proxy_buffer_size value for large Authentik response headers.                                    |
| `AUTHENTIK_PROXY_BUFFERS`         | `8 16k`                                                                                                               | multisite | no       | NGINX proxy_buffers value for large Authentik response headers.                                        |
| `AUTHENTIK_PASS_IDENTITY_HEADERS` | `no`                                                                                                                  | multisite | no       | Forward Authentik identity headers to upstreams that use trusted-header authentication.                |
| `AUTHENTIK_IDENTITY_HEADERS`      | `X-authentik-username X-authentik-groups X-authentik-entitlements X-authentik-email X-authentik-name X-authentik-uid` | multisite | no       | Space- or comma-separated identity headers to forward when AUTHENTIK_PASS_IDENTITY_HEADERS is enabled. |
